...
exports.standardizeKeys = entities => entities.map(e => deepMapKeys(e, key => key === `ID` ? getValidKey({ key: `id` }) : getValidKey({ key })));
...
exports.excludeUnknownEntities = entities => entities.filter(e => e.wordpress_id); // Excluding entities without ID
...
exports.mapPostsToTagsCategories = entities => {
  ...
  return entities.map(e => {
    if (e.__type === `wordpress__POST`) {
      ...
      if (e.tags.length) {
        e.tags___NODE = e.tags.map(t => tags.find(tObj => t === tObj.wordpress_id).id);
        delete e.tags;
      }
      if (e.categories.length) {
        e.categories___NODE = e.categories.map(c => categories.find(cObj => c === cObj.wordpress_id).id);
        delete e.categories;
      }
    }
    return e;
...
exports.mapEntitiesToMedia = entities => {
  ...
  return entities.map(e => {
    ...
    const isPhotoUrl = filename => _.isString(filename) && photoRegex.test(filename);
    const isPhotoUrlAlreadyProcessed = key => key == `source_url`;
    const isFeaturedMedia = (value, key) => (_.isNumber(value) || _.isBoolean(value)) && key === `featured_media`;
...
const createACFChildNodes = (obj, entityId, topLevelIndex, type, children, createNode) => {
  // Replace any child arrays with pointers to nodes
  _.each(obj, (value, key) => {
    if (_.isArray(value) && value[0] && value[0].acf_fc_layout) {
      obj[`${key}___NODE`] = value.map(v => createACFChildNodes(v, entityId, topLevelIndex, type + key, children, createNode).id);
      delete obj[key];
    }
  });

  const acfChildNode = (0, _extends3.default)({}, obj, {
...
exports.createNodesFromEntities = ({ entities, createNode }) => {
  entities.forEach(e => {
      ...
      _.each(entity.acf, (value, key) => {
        if (_.isArray(value) && value[0] && value[0].acf_fc_layout) {
          entity.acf[`${key}_${entity.type}___NODE`] = entity.acf[key].map((f, i) => {
            ...
            const acfChildNode = createACFChildNodes(f, entity.id + i, key, type, children, createNode);
    ...
    let node = (0, _extends3.default)({}, entity, {
      children,
      parent: null,
      internal: {
        type: e.__type,
